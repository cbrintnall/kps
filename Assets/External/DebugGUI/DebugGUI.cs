...
public class DebugGUI : MonoBehaviour
{
    ...
    public static void SetGraphProperties(
        object key,
        string label,
        float min,
        float max,
    ...
    private void InstanceSetGraphProperties(
        object key,
        string label,
        float min,
        float max,
    ...
    void Update()
    {
        ...
        if (LogsEnabled)
        {
            ...
            while (
                transientLogs.Count > 0
                && transientLogs.Peek().expiryTime <= Time.realtimeSinceStartup
            )
            {
                ...
                if (mb != null && attributeKeys.ContainsKey(mb))
                {
                    ...
                    foreach (var key in attributeKeys[mb])
                    {
                        if (key.memberInfo is FieldInfo)
                        {
                            ...
                            object val = fieldInfo.GetValue(mb);
                            if (val == null)
                                return;
                            if (val is int ival)
                                graphDictionary[key].Push(Convert.ToSingle(ival));
                        }
                        else if (key.memberInfo is PropertyInfo)
                        {
                            PropertyInfo propertyInfo = key.memberInfo as PropertyInfo;
                            object val = propertyInfo.GetValue(mb, null);
                            if (val == null)
                                return;
        ...
        for (int i = 0; i < attributeContainers.Count; i++)
        {
            ...
            if (mb != null)
            {
                ...
                if (debugGUIPrintFields.ContainsKey(type))
                {
                    ...
                    {
                        DrawLabel(
                            string.Format("{0} {1}: {2}", mb.name, field.Name, field.GetValue(mb))
                        );
                    }
                }
    ...
    void DrawGraphs()
    {
        ...
        foreach (GraphContainer graph in graphDictionary.Values)
        {
            if (graphGroupBoxesDrawn.Add(graph.group))
            {
                GUI.Box(
                    new Rect(
                        Screen.width - graphWidth,
                        0 + graphBlockHeight * graph.group,
                        graphWidth,
                        graphHeight
                    ),
                    "",
        ...
        foreach (int group in graphGroupBoxesDrawn)
        {
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    GUI.Label(
                        new Rect(
                            Screen.width - graphWidth - minMaxXOffset,
                            groupOrigin,
                            minMaxXOffset,
                            graphHeight
                        ),
                    ...
                    GUI.Label(
                        new Rect(
                            Screen.width - graphWidth - minMaxXOffset,
                            groupOrigin + graphHeight - textSize.y,
                            minMaxXOffset,
                            graphHeight
                        ),
                        labelGuiContent,
                    ...
                    GUI.Label(
                        new Rect(
                            Screen.width - graphWidth - xOffset,
                            yOffset,
                            xOffset,
                            graphHeight
                        ),
        ...
        foreach (var group in graphGroupBoxesDrawn)
        {
            ...
            if (
                ...
                && mousePos.x > Screen.width - graphWidth
                && mousePos.y > group * graphBlockHeight
                && mousePos.y < group * graphBlockHeight + graphHeight
            )
            {
                ...
                GUI.Box(
                    new Rect(mousePos.x, group * graphBlockHeight, 1, graphHeight),
                    "",
                    boxStyle
                );
                ...
                foreach (GraphContainer graph in graphDictionary.Values)
                {
                    if (graph.group == group)
                    {
                        ...
                        GUI.Label(
                            new Rect(mousePos.x + -55, group * graphBlockHeight + yOffset, 45, 50),
                            labelGuiContent,
                            minMaxTextStyle
                        );
                        yOffset += 8f;
    ...
    Dictionary<Type, HashSet<FieldInfo>> debugGUIPrintFields =
        new Dictionary<Type, HashSet<FieldInfo>>();
    Dictionary<Type, HashSet<PropertyInfo>> debugGUIPrintProperties =
        new Dictionary<Type, HashSet<PropertyInfo>>();

    // Instances with debugGUIGraph fields
    Dictionary<Type, HashSet<FieldInfo>> debugGUIGraphFields =
        new Dictionary<Type, HashSet<FieldInfo>>();
    Dictionary<Type, HashSet<PropertyInfo>> debugGUIGraphProperties =
        new Dictionary<Type, HashSet<PropertyInfo>>();

    Dictionary<Type, int> typeInstanceCounts = new Dictionary<Type, int>();

    Dictionary<MonoBehaviour, List<AttributeKey>> attributeKeys =
        new Dictionary<MonoBehaviour, List<AttributeKey>>();
    ...
    void RegisterAttributes()
    {
        ...
        foreach (MonoBehaviour mb in sceneObjects)
        {
                ...
                for (int i = 0; i < objectFields.Length; i++)
                {
                    DebugGUIPrintAttribute printAttribute =
                        Attribute.GetCustomAttribute(
                            objectFields[i],
                            typeof(DebugGUIPrintAttribute)
                        ) as DebugGUIPrintAttribute;
                    ...
                    DebugGUIGraphAttribute graphAttribute =
                        Attribute.GetCustomAttribute(
                            objectFields[i],
                            typeof(DebugGUIGraphAttribute)
                        ) as DebugGUIGraphAttribute;
                    ...
                    if (graphAttribute != null)
                    {
                        ...
                        try
                        {
                            value = Convert.ToSingle(objectFields[i].GetValue(mb));
                        }
                        catch (Exception _)
                        {
                            Debug.LogError(
                                string.Format(
                                    "Cannot cast {0}.{1} to float. This member will be ignored.",
                                    monoType.Name,
                                    objectFields[i].Name
                                )
                            );
                        ...
                        GraphContainer graph = new GraphContainer(graphWidth, graphHeight)
                        {
            ...
            // Properties
            {
                PropertyInfo[] objectProperties = monoType.GetProperties(
                    BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance
                );
                        ...
                        if (objectProperties[i].GetValue(mb, null) as float? == null)
                        {
                            Debug.LogError(
                                "Cannot cast "
                                    ...
                                    + " to float. This member will be ignored."
                            );
                            continue;
                        }
        ...
        private void RegenerateGraph()
        {
            ...
            for (int i = 0; i < values.Length - 1; i++)
            {
                DrawLine(
                    ...
                    i,
                    (int)(
                        Mathf.InverseLerp(min, max, values[Mod(currentIndex - i, values.Length)])
                        * graphHeight
                    ),
                    i + 1,
                    (int)(
                        Mathf.InverseLerp(
                            min,
                            max,
                            values[Mod(currentIndex - i - 1, values.Length)]
                        ) * graphHeight
                    ),
                    color
                );
